...
export default async function UserDashboardPage() {
  ...
  let favorites: { id: string; slug: string; title: string; iconEmoji: string; iconUrl: string | null }[] = [];

  try {
    const [u, w, sessions, favRows] = await Promise.all([
      ...
      // UsageLedger ではなく ChatSession を使う（無料枠の会話も含める）
      prisma.chatSession.findMany({
        ...
        select: {
          ...
          updatedAt: true,
          agent: {
            select: { slug: true, title: true, iconEmoji: true, iconUrl: true },
          },
        },
      }),
    ...
    user = u;
    wallet = w;
    favorites = favRows.map((r) => ({ ...r.agent, iconUrl: r.agent.iconUrl ?? null }));
    ...
    for (const row of sessions) {
      ...
      recentLines.push({
        ...
        iconUrl: row.agent.iconUrl ?? null,
        ...
        <section className="mt-10">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
            {favorites.map((a) => (
              ...
              >
                <span className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-slate-100 text-2xl dark:bg-slate-800">
                  {a.iconUrl ? (
      ...
      <div className="mt-10 flex flex-col gap-8">
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/80 dark:shadow-none sm:p-8">
            <div className="flex items-center gap-2.5 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
              <Wallet className="h-4 w-4 text-[var(--accent)] dark:text-[var(--accent)]" aria-hidden />
              残高 (pt)
            </div>
            <p className="mt-5 text-5xl font-extrabold tabular-nums tracking-tight text-[var(--accent)] dark:text-[var(--accent)]">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/80 dark:shadow-none sm:p-8">
            <div className="flex items-center gap-2.5 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
              <MessageSquare className="h-4 w-4 text-[var(--accent)] dark:text-[var(--accent)]" aria-hidden />
              最近の会話
            </div>
            ...
            {recentLines.length === 0 ? (
              <div className="mt-8 flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/80 px-6 py-12 text-center dark:border-slate-700 dark:bg-slate-950/50">
                <div
                  className="flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-[var(--accent)]/15 to-[var(--accent)]/5 ring-1 ring-[var(--accent)]/20 dark:from-[var(--accent)]/15 dark:to-[var(--accent)]/5"
                  aria-hidden
                >
                ...
                <Link
                  href="/"
                  className="mt-6 inline-flex items-center justify-center rounded-xl bg-[var(--accent)] px-6 py-3 text-sm font-semibold text-white shadow-md shadow-[var(--accent)]/20 transition hover:bg-[var(--accent-hover)] dark:bg-[var(--accent)] dark:hover:bg-[var(--accent-hover)]"
                >
                  エージェントを見る
                </Link>
              </div>
              ...
              <ul className="mt-6 divide-y divide-slate-100 dark:divide-slate-800">
                {recentLines.map((a) => (
                  <li key={a.slug}>
                    ...
                    >
                      <span
                        className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-slate-100 text-xl dark:bg-slate-800"
                        aria-hidden
                      >
                        {a.iconUrl ? (
                          // eslint-disable-next-line @next/next/no-img-element
                      ...
                      <ChevronRight
                        className="h-5 w-5 shrink-0 text-slate-400 transition group-hover:text-[var(--accent)] dark:text-slate-500 dark:group-hover:text-[var(--accent)]"
                        aria-hidden
                      />
                    </Link>
                  </li>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            <Link
              href="/wallet#charge"
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--accent)] px-6 py-4 text-[15px] font-semibold text-white shadow-lg shadow-[var(--accent)]/20 transition hover:bg-[var(--accent-hover)] sm:w-auto sm:min-w-[280px] dark:bg-[var(--accent)] dark:hover:bg-[var(--accent-hover)]"
            >
