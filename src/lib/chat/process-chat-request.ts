...
/**
 * Phase 4 refactor: the entire streaming chat pipeline now lives here,
 * parameterized by `userId` (resolved by the caller) and `allowGuest`
 * (web route allows 3/day IP-based guest chats; `/api/v1/chat` does not).
 *
 * The body is byte-for-byte identical to the previous inline implementation
 * in `src/app/api/chat/route.ts` — the refactor only pulls the user-lookup
 * out. Existing web cookie-based behavior is unchanged.
 */
...
export type ProcessChatRequestOptions = {
  /** Caller-resolved Prisma `User.id`. Null means the caller didn't have a
   *  session and `allowGuest` decides whether to try the guest path. */
  userId: string | null;
  /** When true, missing `userId` falls back to the IP-hashed guest limit
   *  (web `/api/chat`). When false, missing `userId` means 401 immediately
   *  (`/api/v1/chat`). */
  allowGuest: boolean;
};
...
export async function processChatRequest(
  req: Request,
  opts: ProcessChatRequestOptions,
): Promise<Response> {
  let json: unknown;
  try {
    json = await req.json();
  ...
  const result = streamText({
    ...
    onFinish: async ({ totalUsage }) => {
      ...
      if (!charge.ok) {
        console.error(
          "[chat] billing failed after stream:",
          charge.code,
          { userId, modelId },
        );
      }
